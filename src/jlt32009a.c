 ...
 *
 * ── Adapted for Maix Dock (K210_AI_V7s_Plus) ─────────────────────────────────
 * Upstream (kendryte-freertos-demo/lcd) relies on a board-level g_fpioa_cfg
 * table + bsp_pin_setup() to route the pins and enable the SPI0→DVP data path.
 * This project doesn't wire bsp_pin_setup(), so tft_hard_init() programs FPIOA,
 * sysctl_set_spi0_dvp_data(1), the hardware reset pulse and the backlight here.
 */
#include <devices.h>
#include <fpioa.h>
#include <sysctl.h>
...
/* Route the LCD pins and enable the SPI0 OCTAL "8080" data path.
 * D0..D7 are hardwired to the DVP data pads inside the K210 die; the sysctl
 * bit routes SPI0 octal output there, so they need no fpioa mapping. */
static void pins_init(void)
{
    fpioa_set_function(LCD_CS_IO,  FUNC_SPI0_SS3);                /* CS  */
    fpioa_set_function(LCD_WR_IO,  FUNC_SPI0_SCLK);              /* WR  */
    fpioa_set_function(LCD_DCX_IO, FUNC_GPIOHS0 + DCX_GPIONUM);  /* DCX */
    fpioa_set_function(LCD_RST_IO, FUNC_GPIOHS0 + RST_GPIONUM);  /* RST */
    fpioa_set_function(LCD_BL_IO,  FUNC_GPIOHS0 + BL_GPIONUM);   /* BL  */
...
void init_dcx()
{
    ...
    gpio_set_drive_mode(gio, BL_GPIONUM, GPIO_DM_OUTPUT);
    gpio_set_pin_value(gio, BL_GPIONUM, GPIO_PV_HIGH);   /* backlight on */
}
