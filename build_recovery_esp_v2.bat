...
if not exist "%ESP_BUILD%\esp8285-sta-klink.bin" (
...
set "ESP_PART_CMAKE=%ESP_BUILD%\partitions_1mb_singleapp.bin"
set "ESP_APP_CMAKE=%ESP_BUILD%\esp8285-sta-klink.bin"
...
cmake -S firmware_v2\recovery_esp -B "%BUILD%" -G "MinGW Makefiles" -DCMAKE_MAKE_PROGRAM="%MAKE%" -DTOOLCHAIN="%TC%" -DESP_V2_BUILD_DIR="%ESP_BUILD_CMAKE%" -DESP_BOOT_BIN="%ESP_BOOT_CMAKE%" -DESP_PART_BIN="%ESP_PART_CMAKE%" -DESP_APP_BIN="%ESP_APP_CMAKE%" -DESP_PHY_INIT_BIN="%ESP_PHY_CMAKE%" -DESP_SYS_PARAM_BLANK_BIN="%ESP_BLANK_CMAKE%" -DRECOVERY_BOOT_MARKER="STA_READY ssid=ELECTRONICS" -DCMAKE_POLICY_VERSION_MINIMUM=3.5
